...
def create_app(test_config=None):
    ...
    def get_categories():
        try:
            ...
            categories_formatted_to_dict = {
                category.id: category.type for category in categories
            }

            return jsonify(
                {"success": True, "categories": categories_formatted_to_dict}
            )
    ...
    of the screen for three pages.Clicking on the page numbers
    ...
    def delete_question(question_id):
        try:
            equal_check = question_id == Question.id
            question = Question.query.filter(equal_check).one_or_none()
            if question is None:
                abort(404)
            question.delete()
    ...
    def add_question():
        ...
        question = question_details.get("question", None)
        answer = question_details.get("answer", None)
        category = question_details.get("category", None)
        difficulty = question_details.get("difficulty", None)
        ...
        try:
            ...
            question_list = Question.query.order_by(Question.id).all()
            page_questions = paginate_questions(request, question_list)
    ...
    def search_questions():
        ...
        try:
            ...
            page_questions = paginate_questions(request, search_result)
    ...
    def get_questions_by_category(category_id):
        check_equals = Category.id == category_id
        categories_selection = Category.query.filter(check_equals).all()
        if len(categories_selection) == 0:
            abort(404)

        questions_by_category_selection = (
        ...
        return jsonify(
            {
                ...
                "questions": paginate_questions(
                    request, questions_by_category_selection
                ),
